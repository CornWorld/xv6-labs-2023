...
void proc_freekpagetable(pagetable_t p, uint64 kstack)
{
  uvmunmap(p, UART0, 1, 0);
  uvmunmap(p, VIRTIO0, 1, 0);
  uvmunmap(p, CLINT, 0x10000 / PGSIZE, 0);
  uvmunmap(p, PLIC, 0x400000 / PGSIZE, 0);
  uvmunmap(p, KERNBASE, (PHYSTOP-KERNBASE) / PGSIZE, 0);
  uvmunmap(p, TRAMPOLINE, 1, 0);
  uvmunmap(p, kstack, 1, 1);
  uvmfree(p, 0);
}
